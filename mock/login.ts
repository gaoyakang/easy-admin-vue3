...
export default [
  ...
  // 获取用户信息
  {
    ...
    response: (request) => {
      ...
      //如果有返回成功信息
      // routes/buttons/roles/name/avatar
      const { routes, buttons, roles, name, avatar } = checkUser;
      return { code: 200, data: { routes, buttons, roles, name, avatar } };
    },
  },
  // 退出登陆
  {
    ...
    response: (request) => {
      //如果有返回成功信息
      return { code: 200, data: { message: '退出登陆成功' } };
    },
  },
];
